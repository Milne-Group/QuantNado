...
def _resolve_chunk_len(
    ...
    logger.info(
        ...
        resolved, fs_label, fs_probe_path, estimate["num_chunks"],
    )
    return resolved
...
class BamStore(BaseStore):
    ...
    def _init_store(self) -> None:
        ...
        self.meta.create_array("sparsity", shape=(self.n_samples,), dtype=np.float32, fill_value=np.nan, overwrite=True)
        self.meta.create_array("sample_hashes", shape=(self.n_samples, 16), dtype=np.uint8, fill_value=0, overwrite=True)
        self.meta.create_array("total_reads", shape=(self.n_samples,), dtype=np.int64, fill_value=0, overwrite=True)
        self.meta.create_array("mean_read_length", shape=(self.n_samples,), dtype=np.float32, fill_value=np.nan, overwrite=True)
    ...
    def _write_sample_store(
        ...
        if max_workers <= 1 or len(contigs) <= 1:
            for contig, size in self.chromsizes.items():
                sparsity, fwd, rev = self._process_chromosome(
                    bam_file, contig, size, is_stranded,
                    use_fragment=use_fragment, read_filter=read_filter,
                )
                if rev is not None:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as pool:
                futures = {
                    pool.submit(
                        ...
                        bam_file, contig, self.chromsizes[contig], is_stranded,
                        use_fragment=use_fragment, read_filter=read_filter,
                    ): contig
        ...
        tmp_root.attrs.update({
            ...
            "mean_read_length": mean_read_length if mean_read_length is not None else float("nan"),
            "bam_hash": bam_hash,
        })
    ...
    def process_samples(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for sample_idx, (bam_file, sample_name) in enumerate(
                ...
                tmp_path = tmp_dir / f"sample_{sample_idx}.zarr"
                self._write_sample_store(bam_file, sample_name, tmp_path, max_workers=max_workers)
    ...
    def from_bam_files(
        ...
        *,
        ...
        chromsizes_dict = _parse_chromsizes(chromsizes_raw, filter_chromosomes=filter_chromosomes, test=test)
